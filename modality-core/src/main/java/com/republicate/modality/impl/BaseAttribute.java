...
public abstract class BaseAttribute extends InstanceProducer implements Serializable
{
    ...
    protected void initialize()
    {
        ...
        // Check for parameter name collisions with entity column names
        // (skip internal attributes that intentionally use column names as parameters)
        if (parent instanceof Entity && parameterNames.size() > 0 && !isInternalAttribute())
        {
            Entity parentEntity = (Entity) parent;
            List<String> entityColumnNames = parentEntity.getColumnNames();
            // Only check if entity has columns (might be initialized later for non-table entities)
            if (entityColumnNames != null && !entityColumnNames.isEmpty())
            {
                Set<String> columnNames = new HashSet<>(entityColumnNames);
                for (String paramName : parameterNames)
                {
                    if (columnNames.contains(paramName))
                    {
                        getModel().getLogger().warn("attribute {}.{}: parameter '{}' collides with entity column name; " +
                            "if called on an instance, value will come from instance data, not from passed parameters",
                            parentEntity.getName(), getName(), paramName);
                    }
                }
            }
        }
    ...
    /**
     * Check if this attribute should skip parameter collision validation.
     * Internal auto-generated attributes intentionally use column names
     * as parameters for entity operations.
     */
    protected boolean isInternalAttribute()
    {
        return internal;
    }

    /**
     * Mark this attribute as internal (auto-generated by reverse engineering).
     * Internal attributes skip parameter collision validation.
     */
